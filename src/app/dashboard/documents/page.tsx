...
import { 
  File as FileIcon, Plus, ArrowUp, ArrowDown, Trash2, ToggleLeft, ToggleRight, 
...
export default function DocumentsManagerPage() {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    // Enforce Pro constraints on files: Free plan limits to 5 Documents
    if (subType !== "pro" && links.length >= 5) {
      setErrorMsg("You have reached the limit of 5 Documents for the Free Plan. Please upgrade to Pro for unlimited uploads.");
      return;
    }
    ...
    if (uploadFile.size > maxSizeBytes) {
      setErrorMsg("File too large. Maximum allowed size is 25MB.");
      return;
    }
    ...
    try {
      ...
      setNewTitle("");
      setUploadFile(null);
      setSuccessMsg("Document successfully added!");
      setTimeout(() => setSuccessMsg(""), 4000);
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.message || "Failed to add document. Try again.");
    } finally {
    ...
    <div className="space-y-8 select-none max-w-6xl">
      ...
      <div className="border-b border-zinc-200 pb-6 flex flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1 font-medium">Upload and manage documents on your public profile page.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 rounded-[24px] border border-zinc-200 bg-white p-6 lg:p-8 shadow-sm sticky top-24">
          ...
          {subType !== "pro" && links.length >= 5 ? (
            <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-5 text-center">
              <p className="text-sm text-zinc-600 font-medium mb-4">You have reached the limit of 5 Documents for the Free Plan.</p>
              <Link
                href="/pricing"
            ...
            <form onSubmit={handleAddItem} className="space-y-5">
              ...
              {/* Upload File */}
              <div>
                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Select Document File</label>
                <div className="relative rounded-xl border-2 border-dashed border-zinc-200 bg-zinc-50 p-8 text-center hover:bg-zinc-100 hover:border-zinc-300 transition-all">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-bold text-zinc-700">
                      {uploadFile ? uploadFile.name : "Drop file here"}
                    </p>
                    <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wide">
                      Max: 25MB (Doc, Text, Sheets, etc.)
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                {actionLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Plus className="h-5 w-5" /> Upload Document</>}
              </button>
            </form>
          )}
        </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {filteredLinks.length === 0 ? (
            <div className="rounded-[24px] border border-zinc-200 bg-white p-16 text-center text-zinc-500 flex flex-col items-center shadow-sm">
              ...
              <p className="font-bold text-zinc-900 text-lg">No documents found.</p>
              <p className="text-sm mt-1 text-zinc-500 font-medium">Try adjusting your search or uploading a new document.</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0 flex-1 pr-4">
                        ...
                        <p className="text-xs text-zinc-500 font-medium truncate mt-0.5">
                          {link.media_url || "Uploaded Document"}
                        </p>
                    ...
                    <div className="flex items-center justify-end gap-1.5 sm:gap-2 w-auto" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={(e) => {
                          ...
                          if (link.media_url) {
                            ...
                            setSuccessMsg("Document URL copied!");
                      ...
                      <button
                        ...
                        title={link.is_enabled ? "Disable Document" : "Enable Document"}
                      >
                        {link.is_enabled ? (

...
import { 
  FileText as FileIcon, Plus, ArrowUp, ArrowDown, Trash2, ToggleLeft, ToggleRight, 
  Loader2, AlertCircle, Copy, Search, Filter
} from "lucide-react";
import { supabase } from "@/lib/supabase";
import Link from "next/link";

export default function PdfsManagerPage() {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    // Enforce Pro constraints on files: Free plan limits to 5 PDFs
    if (subType !== "pro" && links.length >= 5) {
      setErrorMsg("You have reached the limit of 5 PDFs for the Free Plan. Please upgrade to Pro for unlimited uploads.");
      return;
    }
    ...
    try {
      ...
      setNewTitle("");
      setUploadFile(null);
      setSuccessMsg("PDF successfully added!");
      setTimeout(() => setSuccessMsg(""), 4000);
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.message || "Failed to add PDF. Try again.");
    } finally {
    ...
    <div className="space-y-8 select-none max-w-6xl">
      ...
      <div className="border-b border-zinc-200 pb-6 flex flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1 font-medium">Upload and manage PDF documents on your public profile page.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 rounded-[24px] border border-zinc-200 bg-white p-6 lg:p-8 shadow-sm sticky top-24">
          ...
          {subType !== "pro" && links.length >= 5 ? (
            <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-5 text-center">
              <p className="text-sm text-zinc-600 font-medium mb-4">You have reached the limit of 5 PDFs for the Free Plan.</p>
              <Link
                href="/pricing"
            ...
            <form onSubmit={handleAddItem} className="space-y-5">
              ...
              {/* Upload File */}
              <div>
                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Select PDF File</label>
              ...
              >
                {actionLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Plus className="h-5 w-5" /> Upload PDF</>}
              </button>
            </form>
          )}
        </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {filteredLinks.length === 0 ? (
            <div className="rounded-[24px] border border-zinc-200 bg-white p-16 text-center text-zinc-500 flex flex-col items-center shadow-sm">
              ...
              <p className="font-bold text-zinc-900 text-lg">No PDFs found.</p>
              <p className="text-sm mt-1 text-zinc-500 font-medium">Try adjusting your search or uploading a new PDF.</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end gap-1.5 sm:gap-2 w-auto" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        title={link.is_enabled ? "Disable PDF" : "Enable PDF"}
                      >
                        {link.is_enabled ? (

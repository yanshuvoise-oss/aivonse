...
import { 
  ...
  Share, Bell, CheckCircle2, Sparkles, UserPlus, Mail, Phone, Send, MessageSquare, Globe
} from "lucide-react";
...
export default function SmartSharePage() {
  ...
  const [smartLinkName, setSmartLinkName] = useState("");
  ...
  useEffect(() => {
    ...
    const loadSmartLinkData = async () => {
      // 1. Fetch smart link details (polyfilled via links table)
      const { data: rawSmartLinkData, error: smartLinkError } = await supabase
        .from("links")
        .select("*")
        .eq("type", "smart_link")
        .eq("url", slug.toLowerCase())
        .single();

      if (smartLinkError || !rawSmartLinkData) {
        setLoading(false);
        return;
      }

      const config = rawSmartLinkData.media_url ? JSON.parse(rawSmartLinkData.media_url) : {};
      const smartLinkData = {
        id: rawSmartLinkData.id,
        profile_id: rawSmartLinkData.profile_id,
        slug: rawSmartLinkData.url,
        name: rawSmartLinkData.title,
        config: config,
        views: rawSmartLinkData.sort_order || 0
      };

      setSmartLinkName(smartLinkData.name);
      const categories = config?.categories || {};
      const selectedItemIds = new Set(config?.selectedItemIds || []);

      // 2. Fetch profile details
      const { data: profileData } = await supabase
        .from("profiles")
        .select("*")
        .eq("id", smartLinkData.profile_id)
        .single();
      ...
      await supabase.from("analytics").insert({
        profile_id: smartLinkData.profile_id,
        link_id: null,
        event_type: "view",
        device_type: device,
        referral_source: `smart_share:${slug}`
      });
      ...
      const { data: linksData } = await supabase
        .from("links")
        .select("*")
        .eq("profile_id", smartLinkData.profile_id)
        .eq("is_enabled", true)
        .order("sort_order", { ascending: true });

      let parsedLinks: any[] = [];
      if (linksData) {
        parsedLinks = linksData.filter((link: any) => {
          if (config?.is_full_content) return true;

          if (!selectedItemIds.has(link.id)) return false;
          
          if (link.type === 'link' && !categories.link) return false;
          if (link.type === 'pdf' && !categories.pdf) return false;
          if (link.type === 'image' && !categories.image) return false;
          if (link.type === 'document' && !categories.document) return false;
      ...
      // 5. Fetch Analytics for total views
  ...
  if (!profile) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#F9F9FB] px-4 text-center select-none text-zinc-900">
        ...
        <h2 className="text-2xl font-extrabold text-zinc-900">Share Link Not Found</h2>
        <p className="mt-2 text-sm text-zinc-500 max-w-xs">
          This smart share link is invalid or has been removed.
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F9F9FB] text-zinc-900 font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12 space-y-16">
        ...
        <section className="flex flex-col items-center text-center space-y-5">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-600 text-xs font-semibold mb-2">
